...
source_files := Dockerfile $(shell find . -type f ! -path './dist/*' ! -path './dep/*' ! -path './.dep-*' ! -path './Dockerfile' ! -path './security-issues-from-bandit.csv' ! -path './requirements.txt' ! -path './vulnerabilities-pip-audit.txt' ! -path './.venv/*')
