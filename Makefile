...
DEP_HASH := $(shell find . -type f \( -path './update-dep.Dockerfile' -o -path './pip-requirements.txt' -o -path './pyproject.toml' -o -path './src/pixsaw/*' -o -path './update-dep-run-audit.sh' -o -path './update-dep.sh' \) -exec md5sum '{}' \; | sort | md5sum - | cut -d' ' -f1)
